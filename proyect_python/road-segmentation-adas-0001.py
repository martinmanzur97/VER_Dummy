...
class Udf:
    ...
    def __init__(self, model_xml, model_bin, device, confidence_threshold, paint):
        ...
        # background: black, road: kind of blue, curb: red, lanemark: green
        self.colormap = np.array(
            [
                BGRColors.BLACK.value,
                BGRColors.BLUE_KIND.value,
                BGRColors.RED.value,
                BGRColors.GREEN.value,
            ]
        )
    ...
    def process(self, frame, metadata):
        ...
        """
        self.log.debug(f"Entering road segmentation Udf::process() function...")
        ...
        if not self.requested:
            self.exec_net.requests[0].async_infer({self.input_layer_ir: input_image})
            self.requested = True

        if self.exec_net.requests[0].wait(0) == StatusCode.OK:
            self.requested = False
            result = self.exec_net.requests[0].output_blobs[self.output_layer_ir].buffer
            ...
            self.resized_mask = cv2.resize(mask, (image_w, image_h))

        if self.paint and self.resized_mask.any():
            # Create image with mask put on
            image_with_mask = cv2.addWeighted(
                self.resized_mask, self.alpha, frame, 0.8, 0
            )
